...
test('tracing with parameterized rules', function(t) {
  ...
  t.equal(trace.succeeded, true);
  t.equal(trace.pos, 0);
  t.equal(trace.cst.ctorName, 'start');
  t.equal(trace.cst.numChildren(), 1);
  ...
  t.equal(app.cst.ctorName, 'foo');
  t.equal(app.cst.numChildren(), 1);
  ...
  t.equal(alt.children[0].succeeded, false);
  t.equal(alt.children[1].succeeded, true);
  t.equal(alt.cst.isIteration(), true);
  t.equal(alt.cst.numChildren(), 5);

  var many = alt.children[1];
  t.equal(many.displayString, 'letter*');
  t.equal(many.interval.contents, 'hallo');
  t.equal(many.children.length, 6);
  t.equal(many.cst.ctorName, '_iter');
  t.equal(many.cst.numChildren(), 5);
...
test('tracing with memoization', function(t) {
  ...
  t.equal(many.cst.isIteration(), true);
  ...
  t.equal(many.children[0].cst.ctorName, 'letter');
  t.equal(many.children[0].cst.numChildren(), 1);
  ...
  t.equal(many.children[1].cst.ctorName, 'letter');
  t.equal(many.children[1].cst.numChildren(), 1);
...
test('tracing with left recursion', function(t) {
  ...
  t.equal(recApply.cst.ctorName, 'id_rec');
  t.equal(recApply.cst.numChildren(), 2);
  ...
  t.equal(seq.children[0].cst.ctorName, 'id');
  t.equal(seq.children[0].cst.numChildren(), 1);
  t.equal(seq.children[0].children[0].cst.ctorName, 'id_rec');
  t.equal(seq.children[0].children[0].cst.numChildren(), 2);
...
test('memoization', function(t) {
  ...
  t.equal(applyId.cst.ctorName, 'id');
  t.equal(applyId.cst.interval.startIdx, 0);
  t.equal(applyId.cst.interval.endIdx, 2);
  ...
  t.equal(applyId2.cst.ctorName, 'id');
  t.equal(applyId2.cst.interval.startIdx, 0);
  t.equal(applyId2.cst.interval.endIdx, 2);
